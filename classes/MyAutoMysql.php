...
/*
 simple mysql libary, should partially be rewritten using mysqli or pdo because
 mysql_* functions will be dropped
  ...
  // example query:
  $db->printf('SELECT * FROM X where A = ?i AND ?sfield_in', ['name', ["alice","bob"]]);
  $db->query('SELECT * FROM X where A = ?i AND ?sfield_in', ['name', ["alice","bob"]]);

  $db->autoInsert('table', ['name' => 'new name']);
  $db->autoUpdate('table', ['name' => 'new name'], ['name' => 'old_name']);
*/
...
class MyMysqlDB extends MyDB {
	...
	public function quoteNachTyp($v, $typ = ''){
		...
		if (substr($typ, 1, strlen('field_in')) == 'field_in') {
      $typ = substr($typ,0,1);
      $field_in = true;
    }
		if (substr($typ, 1, strlen('field_like_any')) == 'field_like_any') {
      $typ = substr($typ,0,1);
      $field_like_any = true;
    }
    ...
    if (isset($field_like_any)){
      ...
      if (count($values) == 0){
        ...
        $name = is_array($name)
          ? implode('.', array_map(function($x){ return '`'.$x.'`'; }, $name))
          : '`'.$name.'`';
        return SQLQuery::ors(array_map(function($v)use(&$name, &$qf){
          global $db;
          return $name .' LIKE '.call_user_func(array($db, $qf), $v);
        }, $values));
      }
    }
    if (isset($field_in)){
      ...
      if (count($values) == 0){
        ...
        $name = is_array($name)
          ? implode('.', array_map(function($x){ return '`'.$x.'`'; }, $name))
          : '`'.$name.'`';
        return $name.' IN ('.implode( array_map(array($this, $qf), $values),', ').')';
      }
    } else if (isset($in)){
	...
	public function printf(){
		...
		preg_match_all('/(^|\?(?:dt|[tobnfidsv])?(?:in|field_in|field_like_any)?)((?::[^\t\r\n) ,:]+)*)([^?]*)/', $sql, $matches);
		...
		for ( $i = 1; $i < sizeof($matches[0]); $i++){
			...
			$type = substr($matches[1][$i], 1, 999); // substr($matches[1][$i], 1, 3);
    ...
    protected function error($sql, $mysql_error_text){
        throw new DBError(pre("SQL Statement war\n$sql\n\n Fehler:\n".mysql_error($this->db_conn)));
    }
    ...
    protected function _q($sql){
        ...
        $time		= microtime(true); 
			...
			if (LOG_MYSQL_ABFRAGEN === true){
                          $debug_handle = fopen('php.log', 'a+');
                          fwrite($debug_handle, "warten auf : >>> ".print_r($sql,true)."\n" );
                          fclose($debug_handle);
			}
    ...
    protected function _rufeAufMit($f, $args){
        $sql = call_user_func_array(array($this,'printf'), $args);
	...
	public function quoteString($s){ return "'".mysql_real_escape_string($s)."'"; }

	public function quoteBoolean($b){ return $b ? '1' : '0'; }

	public function quoteInt($i){
                if (is_array($i))
                  throw new Exception('array unexpected');
		if ($i != $i*1)
	...
	public function quoteDateTime($d, $quote=true){
          ...
          if (preg_match('/^(\p{Nd}{1,2})\.(\p{Nd}{1,2})\.(\p{Nd}{1,4})(\s+(\p{Nd}{1,2}):(\p{Nd}{1,2}):(\p{Nd}{1,2}))?$/',$d, $m)){
            return  $quote
            ...
            return  $quote
              ...
              throw new ActionErrorException( 'Fehler: Tag.Monat.Jahr Stunde:Minute erwartet, aber "'.$d.'" bekommen. Jahr und Uhrzeit sind optional');
	}
...
class MyAutoMysqlDB extends MyMysqlDB {
    ...
    public function dumpTable($table, $orderby = null, $was){
        ...
        $sql_rows = array();
        foreach( $rows as $row){
			$sql_rows[] = $this->printf($value_Platzhalter, $row);
		}
